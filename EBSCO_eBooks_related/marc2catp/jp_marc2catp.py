...
#
# 20181003JP:
#  245/880フィールドに$pがあった場合、$aに連結してタイトルを生成するようにした
#  ただし245$pがあり880$pがない場合にはかな変換されずに残ってしまう
#
# 20180911JP:
#  ひらがなではなくデフォルトでカタカナ変換に変更。カタカナかひらがなかを選択できるようにした
#
# 20180905JP:
#  non-jp版 20180905の変更を反映
...
def romaji_conv( romaji, enable_rom2kana, kana_conv ):
  if enable_rom2kana:
    if kana_conv == 'kata':
      return( unicode( rom2kana.romaji2katakana( romaji.encode( 'utf-8' ) ), 'utf-8' ) )
    else:
      return( unicode( rom2kana.romaji2hiragana( romaji.encode( 'utf-8' ) ), 'utf-8' ) )
  else:
    return( romaji )

def convert_kana( kanji, romaji, kana_conv ):
  # 元の漢字とローマ字を比較して漢字の方に同じ単語が含まれていたら、その単語はかな変換対象から外す
  # ' = ' 以降は英語の題名が入っていることが多いので、ここは比較対象から外す
  kanji1 = kanji.split( ' = ' )
  romaji1 = romaji.split( ' = ' )
  arr1 = romaji1[0].split()
  arr2 = []
  for n in range( len( arr1 ) ):
    ...
    if s1.endswith( '.' ):
      s2 = s1[:-1]
    if kanji1[0].find( s2 ) < 0:
      arr2.append( romaji_conv( s1, True, kana_conv ) )
    else:
      arr2.append( s1 )
  if len( romaji1 ) > 1:
    arr2.extend( [ '=', romaji1[1] ] )
  return( u' '.join( arr2 ) )
            ...
            for sf in afield.keys():
              if sf in ( '6', 'l' ):
                ...
                if not sf_replaced.has_key( sf ):
                  rf[ sf ] = afield[ sf ]
# この処理はやらない方がいい？
#            # 245に限りリンク先フィールド（元情報）の方のsubfieldにしかない値を削除し880フィールドのsubfieldに設定
#            if lfield.startswith( '245' ):
#              for sf in rf.keys():
#                if sf in ( '6', 'l' ):
#                  continue
#                else:
#                  if not afield.has_key( sf ):
#                    afield[ sf ] = rf[ sf ]
#                    del( rf[ sf ] )
...
#        print '%s ind1:%s ind2:%s' % ( fcode, ind1, ind2 )
  ...
  def print_a_record( self ):
    ...
    if self.dataset.has_key( '245' ):
      ...
      link_f1 = adat.get( '6', [''] )[0]
      if link_f1 != '':
        ( lfield, ser_num ) = link_f1.split( '/' )[0].split( '-' )
        # lfield: リンク先フィールド番号, ser_num: リンクシリアル番号
        xrfield = '-'.join( ( '245', ser_num ) )
        # xrfield: リンク先フィールド番号のsubfield 6にあるはずのフィールド情報
        if self.dataset.has_key( lfield ):
          for rf in self.dataset[ lfield ]:
            if rf.get( '6', [''] )[0].split( '/' )[0] == xrfield:
              # 880フィールドの方のsubfieldの値でタイトルのヨミを生成
              trr1a = rf.get( 'a', [''] )[0]
              trr1b = rf.get( 'b', [''] )[0]
              trr1c = rf.get( 'c', [''] )[0]
              trr1p = rf.get( 'p', [''] )[0]
              if trr1p != '':
                trr1a = ' '.join( ( trr1a, trr1p ) )
              if self.enable_rom2kana:
                trr1a = convert_kana( tr1a, trr1a, self.kana_conv )
                trr1b = convert_kana( tr1b, trr1b, self.kana_conv )
                trr1c = convert_kana( tr1c, trr1c, self.kana_conv )
              trr1 = trr1a
              if trr1b != '':
                trr1 += ' ' + trr1b
              if trr1c != '':
...
#        if puba + pubb + pubc != '':
              ...
              if self.dataset.has_key( lfield ):
                for rf in self.dataset[ lfield ]:
                  if rf.get( '6', [''] )[0].split( '/' )[0] == xrfield:
                    alar = rf.get( 'a', [''] )[0]
                    if alar != '':
                      s = 'AHDNGR=%s' % ( romaji_conv( alar, self.enable_rom2kana, self.kana_conv ), )
                      if ald != '':
                        s += ' %s' % ( ald, )
                      sline.append( s )
          sline.append( '</AL>' )
  ...
  #      for subd1 in ( 'v', 'x', 'y', 'z' ):
        ...
        if shr1 != '':
          sline.append( 'SHR=%s' % ( convert_kana( shd1, shr1, self.kana_conv ), ) )
          #sline.append( 'SHR=%s' % ( shr1, ) )
          #
          # Subject はひらがなにしなくてもいい？？
          #
        sline.append( '</SH>' )
    #
    # IDENT
